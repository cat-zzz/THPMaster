...
from competition.core.strategy.strategy_by_win_rate import Strategy
...
def play_1(s: socket.socket):
    ...
    while True:
        ...
        game_flag, catzzz_action_flag = game.parse_server_cmd_1(reply)  # 解析服务端指令并更新实体类信息
        # game.print_info()
        if catzzz_action_flag == constants.take_action:  # 需要我方做出动作
            client_cmd = strategy.strategy(i)
            game.parse_client_cmd(client_cmd, catzzz, opponent)
            print("\n我方发送指令: ", client_cmd)
            # time.sleep(1)
