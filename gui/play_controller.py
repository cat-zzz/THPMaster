...
class NoLimitHoldemRunner(QObject):
    ...
    def __init__(self):
        ...
        self.oppo_strategy_worker = None
        # 下面是强化学习环境
        ...
        self.oppo_strategy_thread = QThread()
    ...
    def run(self):
        ...
        # 设置对手策略与多线程
        self.oppo_strategy_worker = StrategyWorker(self.oppo_strategy)
        self.oppo_strategy_worker.moveToThread(self.oppo_strategy_thread)
        self.oppo_strategy_thread.started.connect(self.oppo_strategy_worker.run)
        self.oppo_strategy_worker.finished.connect(self.oppo_strategy_finished)
        # self.oppo_strategy_worker.finished.connect(self.oppo_strategy_thread.quit)
        self.oppo_strategy_worker.finished.connect(self.oppo_strategy_worker.deleteLater)
        self.oppo_strategy_thread.finished.connect(self.oppo_strategy_thread.deleteLater)
        # UI布局
    ...
    def oppo_strategy_finished(self, result):
        """
        OppoStrategyWorker执行完毕后的回调函数
        :param result: OppoStrategyWorker.run()的执行结果
        :return:
        """
        print(f'调用oppo_strategy_finished(), 其oppo_strategy为{result}\n')
        # 做出行动
