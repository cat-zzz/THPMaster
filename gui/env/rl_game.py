...
from gui.util import poker_value, card_util
...
raise动作表示下注到多少筹码，而不是增加了多少筹码，例如：
    p1:[raise, 200], p2:[raise, 400], p1:[raise, 800],p1最终下注800筹码，而不是200+800=1000筹码
"""
...
class NoLimitHoldemGame:
    def __init__(self, game_config=None):
        ...
        self.cur_stage_chip = 0  # 当前阶段所有玩家已下注的筹码量之和
        # valid_bet_chip：如果call动作，则下注量与此值相等，如果是raise动作，则下注量是此值的两倍及以上
        self.valid_bet_chip = 0  # 最新一次有效下注量
        # 新增game_state_flag：与step()函数中的game_state_flag的返回值对应，只通过check_game_state()函数更新其值
    ...
    def step(self, action: np.ndarray):
        ...
        :returns: game_state_flag: 执行action之后的状态标志；preflop只算做一个阶段（即使其他玩家call）
        info: 如果进入了下一阶段，info则保存了隐藏的一次call行动；如果是在earn chip阶段，info则保存了各玩家输赢筹码量
        """
        info = {}
        down = False
        if self.check_action(action):
            ...
            if self.game_state_flag == self.CheckStateFuncResult.enter_next_state:
                ...
                if self.cur_stage == constants.earn_chip_stage:
                    payoffs = self._enter_earn_chip(self.game_state_flag)
                    info['payoffs'] = payoffs
                    down = True
            elif (self.game_state_flag == self.CheckStateFuncResult.folded_enter_earn_chip_stage
                  or self.game_state_flag == self.CheckStateFuncResult.allin_enter_earn_chip_stage):
            ...
            # self.game_state_flag = self.CheckStateFuncResult.stay_cur_stage
        state = self.get_state()
        return state, is_legal, self.game_state_flag, down, info
    ...
    class CheckStateFuncResult(Enum):
        ...
        game_init = constants.func_game_init
...
class Player:
    ...
    def __init__(self, player_idx):
        self.player_idx = player_idx  # 玩家id，从0开始，对应players中的索引，初始化之后不再改变其值
